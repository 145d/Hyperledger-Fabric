...
function queryCommitted() {
  ...
  local COUNTER=1
  # continue to poll
  # we either get a successful response, or reach MAX RETRY(我们要么得到一个成功的响应，要么达到MAX RETRY)
  while [ $rc -ne 0 -a $COUNTER -lt $MAX_RETRY ]; do
    sleep $DELAY
...
queryInstalledOnPeer() {
  ...
  local rc=1
  local COUNTER=1
  # continue to poll(继续投票)
  # we either get a successful response, or reach MAX RETRY(我们要么得到一个成功的响应，要么达到MAX 重试)
  while [ $rc -ne 0 -a $COUNTER -lt $MAX_RETRY ]; do
    #sleep $DELAY
    #infoln "Attempting to list on peer0.org${ORG}, Retry after $DELAY seconds."（"尝试在peer0.org${ORG}上列出，$DELAY秒后重试。"
